...
#define NAME_GAME "Покер на Костях"
...
{
	SG_RegisterGame("poker", NAME_GAME, StartGame_Poker);
}
		...
		if (SG_IsClientPlay(client1))
		{
			if (IsValidClient(client2))
			{
				if(PCubes[client2] == 5)
				{
					...
					SG_FinalGame(client2, NAME_GAME, buffer2, buffer1, true);
					SG_FinalGame(client1, NAME_GAME, buffer2, buffer1);
				}
				else
				{
					SG_ResetGame(client1, client2, true);
					if (IsValidClient(client2))
					{
